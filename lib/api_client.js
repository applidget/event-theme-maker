...
module.exports = class {
   ...
   * @param {string|null} emailId - Email template ID (optional)
   * @param {string|null} documentId - Document template ID (optional)
   * @param {boolean|null} insecure - Disable SSL certificate verification (DEVELOPMENT ONLY - never use in production)
   */
  constructor(endpoint, token, eventId = null, emailId = null, documentId = null, insecure = null) {
    this.token = token;
  ...
  get(path, data, cb) {
    ...
    const req = this.httpClient().request(options, res => {
      ...
      res.on("end", () => {
        cb(res.statusCode, parseResponse(response));
      });
    });
...
const parseResponse = (response) => {
  try {
    ...
    return { error: `unexpected response (status may indicate auth issue): ${response}` };
  }
}
