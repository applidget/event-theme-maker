...
Because themes may depends on other themes, the bundle mimics a repository
structure, for example, if a theme has some dependencies on the standard theme, the bundle will have a structure that looks like:
...
const copyFilesToBundleFolder = (theme, files, bundleDir) => {
  files.forEach(file => {
...
const archiveBundle = (releaseDir, bundleDir, assetsDir, cb) => {
  ...
  exec(cmd, { cwd: releaseDir }, (error) => {
      if (error) throw `Error building tar.gz file: ${error}`;
