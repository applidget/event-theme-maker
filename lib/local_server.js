...
module.exports.startLocalServer = (ngrokHost) => {
  http.createServer(requestHandler(ngrokHost)).listen(NGROK_PORT);
}

function requestHandler(ngrokHost) {
  return (req, res) => {
    ...
    fs.exists(`.${pathname}`, function (exist) {
      ...
      fs.readFile(`.${pathname}`, function(err, data){
        if (err) {
          ...
          res.end(replaceAssetsHost(content, ngrokHost));
        }
      });
    });
  };
