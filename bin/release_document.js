...
const replaceProductionURL = (releaseDir, theme) => {
  ...
  const reg = new RegExp(`https://${DOCUMENT_FAKE_HOST}`, "g");

  replaceInFile(path.join(releaseDir, theme, "specs.yml"), reg, host);
}

const copyDocumentTheme = (theme, releaseDir, templateDir, cb) => {
...
const deleteReleaseDir = () => {
  fs.rmSync(DOCUMENT_BUCKET_ROOT_DIR, { recursive: true, force: true });
}

logInfo(`Starting upload for ${theme}`);
const releaseDir = DOCUMENT_BUCKET_ROOT_DIR;
const templateDir = `document_themes`;
copyDocumentTheme(theme, releaseDir, templateDir, () => {
  logInfo(`Uploading files for ${theme}, templateDir: ${templateDir}, releaseDir: ${releaseDir}`);
  upload(theme, ".", releaseDir, apiClient, false, (files) => {
    files.forEach(f => logUpload(f));
    logSuccess(`Documents ${theme} theme has been uploaded`);
    deleteReleaseDir();
  });
});
